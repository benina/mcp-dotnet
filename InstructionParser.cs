...
public class InstructionParser
{
        ...
        switch (opcode.OperandType)
        {
            ...
            case OperandType.InlineI8:
                if (long.TryParse(operandStr, out var longValue))
                    return longValue;
                throw new ArgumentException($"Invalid int64 operand: {operandStr}");

            case OperandType.InlineR:
                if (double.TryParse(operandStr, out var doubleValue))
                    return doubleValue;
                throw new ArgumentException($"Invalid float64 operand: {operandStr}");

            case OperandType.ShortInlineR:
                if (float.TryParse(operandStr, out var floatValue))
                    return floatValue;
                throw new ArgumentException($"Invalid float32 operand: {operandStr}");

            case OperandType.InlineField:
                return ResolveField(operandStr); // bạn cần định nghĩa ResolveField

            case OperandType.InlineTok:
                return ResolveMember(operandStr); // xử lý loại member phù hợp (field, method, type)

            case OperandType.InlineVar:
            case OperandType.ShortInlineVar:
                if (ushort.TryParse(operandStr, out var varIndex))
                    return varIndex;
                throw new ArgumentException($"Invalid variable index: {operandStr}");

            case OperandType.InlineBrTarget:
            case OperandType.ShortInlineBrTarget:
                if (int.TryParse(operandStr, out var offset))
                    return offset;
    ...
    private IField ResolveField(string fieldStr)
    {
        try
        {
            // Extract assembly name if present: [mscorlib]
            string asmName = null;
            ...
            // Split into type and field name: Namespace.Type::Field
            ...
            if (asmName != null)
            {
                // Find matching assembly reference
                ...
                typeRef = new TypeRefUser(_module, name, ns, new AssemblyRefUser(asmRef));
            }
            else
            {
            ...
            // Create a field reference (type sig unknown, so use CorLibTypes.Object as placeholder)
            return new MemberRefUser(
                _module,
                fieldName,
                new FieldSig(_module.CorLibTypes.Object),
                typeRef);
        }
        catch (Exception ex)
        {
    ...
    private IMemberRef ResolveMember(string memberStr)
    {
        // Method detection: contains "::" AND "("
        if (memberStr.Contains("::") && memberStr.Contains("("))
            return (IMemberRef)ResolveMethod(memberStr);

        // Field detection: contains "::" but no "()"
        if (memberStr.Contains("::"))
            return (IMemberRef)ResolveField(memberStr);

        // Type detection: no "::"
        return (IMemberRef)ResolveType(memberStr);
    }
